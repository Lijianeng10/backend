...
use app\modules\common\models\DealOrder;
...
class BetlistController extends Controller {
    ...
    public function actionIndex() {
        ...
        $detail = (new Query())->select(["lottery_order.*", 's.store_name', 'u.user_tel', 'a.api_order_code', 'a.third_order_code', 'sum(d.win_amount) deal_win_amount'])
                ...
                ->andWhere(['lottery_order.source' => 7]);
         //判断当前登录用户是咕啦内部用户还是合作渠道商户,合作渠道商是本账号还是所属操作账号
        if($session["type"]==1){
            if($session["agent_code"]=="gl00015788"){
                $detail = $detail->andWhere(['lottery_order.cust_no' => $session["admin_name"]]);
            }else{
                $detail = $detail->andWhere(['lottery_order.cust_no' => $session["agent_code"]]);
            }
        }
        ...
        if (isset($get["store_info"])&&!empty($get["store_info"])) {
            $detail = $detail->andWhere(["or", ["s.cust_no" => $get['store_info']], ["s.store_name" => $get['store_info']], ["s.phone_num" => $get['store_info']]]);
        }
        ...
        if (isset($get["out_status"]) && !empty($get['out_status'])) {
            $detail = $detail->andWhere(["in", "lottery_order.status", explode("|", $get["out_status"])]);
        } else {
...
//        $data["lotOrder"]["contents"] = $bets;
        ...
        $dealOrder = [];
        if (in_array($lotOrder['status'], [3, 4, 5])) {
            $schePlay = CompetConst::SCHEDULE_PLAY;
            $playCode = Constants::MANNER;
            $dealOrder = DealOrder::find()->select(['lottery_code', 'play_code', 'bet_val', 'odds', 'bet_money', 'bet_double', 'win_amount', 'status'])->where(['order_id' => $lotOrder['lottery_order_id']])->asArray()->all();
            foreach ($dealOrder as $key => &$deal) {
                $dealBets = explode('|', $deal['bet_val']);
                $betArr = [];
                foreach ($dealBets as $dbets) {
                    preg_match($pattern, $dbets, $res);
                    if ($lotOrder['lottery_id'] != 3011) {
                        $arr = explode(",", $res[2]);
                        $betName = '';
                        foreach ($arr as $a) {
                            $betName .= $schePlay[$lotOrder['lottery_id']][$a] . '，';
                        }
                        $betArr[] = $schedules[$res[1]]['schedule_code'] . '>' . rtrim($betName, '，');
                    } else {
                ...
                $deal['bet'] = implode('</br>', $betArr);
                $playCodeArr = explode(',', $deal['play_code']);
                $playName = [];
                foreach ($playCodeArr as $ii) {
                    $playName[] = $playCode[$ii];
                }
                $deal['play_name'] = implode(',', $playName);
                $deal['statusName'] = $statusNames[$deal['status']];
            }
        }
        $data['deal_order'] = new ArrayDataProvider([
            'allModels' => $dealOrder,
            'pagination' => [
                'pageSize' => count($dealOrder),
            ]
        ]);
...
//        $this->layout = false; 
        ...
        $errorStatus = [6, 9, 10];
        $order = "";
        if (in_array($res['data']["status"], $successStatus)) {
            $order = "create_time asc";
        } else {
            $order = "create_time desc";
        }
        ...
        $data = ["res" => $res['data'], "payInfo" => $payRecord, "content" => $detail, 'dealOrder' => $res['dealOrder']];
...
//        $lotOrder['award_time'] = date('Y-m-d H:i:s', strtotime('+2 hours', $lotOrder['periods'] / 1000));
        ...
        if (in_array($lotOrder['status'], [3, 4, 5])) {
            ...
            foreach ($dealOrder as $key => &$deal) {
                ...
                $deal['play_name'] = implode(',', $playName);
                $deal['statusName'] = $status[$deal['status']];
            }
        }
        $orderData = ['data' => $data, 'dealOrder' => $dealOrder];
        return $orderData;
    }
...
//                    $val['content'][$key]['home_team_name'] = $schedules[$mid]['home_team_name'];
    ...
    public function getBdOrder($lotteryOrderCode) {
        ...
        foreach ($bets as &$val) {
            ...
            $newPlay = array_unique($palyCode);
            foreach ($newPlay as $n) {
                if ($n != 5005) {
                    $val["schedule_result_" . $n] = ($schedule["status"] != 2) ? "" : $schedule["result_" . $n];
                } else {
...
//                $val["content"][$key]["rf_nums"] = $schedules[$mid]["sfgg_rq_nums"];
    ...
    public static function getCupDetial($order) {
        ...
        $field = ['lottery_order.cust_no', 'lottery_order.award_time', 'lottery_order.out_time', 'lottery_order.bet_val', 'lottery_order.lottery_id', 'lottery_order.lottery_name', 'bet_money', 'lottery_order_code', 'lottery_order.create_time', 'lottery_order_id', 'lottery_order.status', 'win_amount', 'play_code', 'play_name', 'bet_double',
            'count', 'lottery_order.periods', 's.store_name', 's.store_code', 's.telephone phone_num', 'odds', 'l.lottery_pic', 'pr.pay_pre_money'];
    ...
    public  function actionPrintReport(){
        ...
        if (isset($get["out_start"])) {
            $detail = $detail->andWhere([">", "lottery_order.out_time", $get["out_start"]. " 00:00:00"]);
        }
        if (isset($get["out_end"])) {
            $detail = $detail->andWhere(["<", "lottery_order.out_time", $get["out_end"] . " 23:59:59"]);
        }
        $detail = $detail->groupBy('lottery_order.lottery_order_id')->orderBy("lottery_order.create_time desc")->all();
        $counts = count($detail);
        if($counts==0){
...
//            Excel::printSzcOrder($detail,$counts);
            echo '<script type="text/javascript">alert("暂未开放该彩种导出功能");window.history.go(-1);</script>';
            exit();
        }else{
            Excel::printChannelJcOrder($detail,$counts);
