...
class AgentReportController extends Controller {
    ...
    public function actionGetReport() {
        ...
        $statusArr = [3, 4, 5];
        $field =["a.agents_name", "a.agents_account","a.agents_id", "count(distinct l.cust_no) count", "sum(l.bet_money) salemoney", "count(l.lottery_order_id) ordernum", "sum(l.win_amount) winmoney","sum(l.award_amount) award_amount",new Expression("sum(case when l.status = 3 then 1 else 0 end) as stayopen"),new Expression("sum(case when l.status = 4 and l.deal_status = 1 then 1 else 0 end) as  stayaward"),"sum(p.pay_money) paymoney"];
        if($type==1){
        ...
        $query = (new Query())->select($field)
            ...
            ->leftJoin("agents as a", "l.agent_id=a.agents_id")
            ->leftJoin("pay_record p","l.lottery_order_code=p.order_code and p.pay_type=1")
            ->where(["l.order_platform"=>2])
            ->andWhere(["in", "l.status", $statusArr]);
        if (isset($post["agentName"]) && !empty($post["agentName"])) {
            $query = $query->andWhere(['or',["a.agents_account" => $post["agentName"]], ["like", "a.agents_name", $post["agentName"]]]);
        }
        ...
        $query = $query->groupBy(["a.agents_name", "days"])
            ->orderBy("a.agents_name desc,days desc");
        $result = $query->all();
    ...
    public function actionGetMonthReport() {
        ...
        $query = $query->groupBy(["a.agents_name", "months"])
            ->orderBy("a.agents_name desc,months desc");
        $result = $query->all();
        return $this->jsonResult(100, "获取成功", $result);
    }
    ...
    public function actionGetLotteryReport() {
        ...
        $field =["a.agents_name", "a.agents_account","a.agents_id", "l.lottery_id","l.lottery_name","count(distinct l.cust_no) count", "sum(l.bet_money) salemoney", "count(l.lottery_order_id) ordernum", "sum(l.win_amount) winmoney","sum(l.award_amount) award_amount",new Expression("sum(case when l.status = 3 then 1 else 0 end) as stayopen"),new Expression("sum(case when l.status = 4 and l.deal_status = 1 then 1 else 0 end) as  stayaward"),"sum(p.pay_money) paymoney"];
        ...
        $query = $query->groupBy(["a.agents_name","l.lottery_id"]);
        $result = $query->all();
        $res =[];
            ...
            foreach ($res as $key =>&$val){
                ...
                foreach ($val as $k =>$v){
                    if (in_array($v["lottery_id"], $lotteryZu)) {
                        ...
                        $res[$key]["3000"]["paymoney"]+=$v["paymoney"];
                        $res[$key]["3000"]["agents_name"]=$v["agents_name"];
                        $res[$key]["3000"]["agents_account"]=$v["agents_account"];
                        unset($val[$k]);
                    }
                    if (in_array($v["lottery_id"], $lotteryLan)) {
                        ...
                        $res[$key]["3100"]["paymoney"]+=$v["paymoney"];
                        $res[$key]["3100"]["agents_name"]=$v["agents_name"];
                        $res[$key]["3100"]["agents_account"]=$v["agents_account"];
                        unset($val[$k]);
                    }
                    if (in_array($v["lottery_id"], $lotteryBd)) {
                        ...
                        $res[$key]["5000"]["paymoney"]+=$v["paymoney"];
                        $res[$key]["5000"]["agents_name"]=$v["agents_name"];
                        $res[$key]["5000"]["agents_account"]=$v["agents_account"];
                        unset($val[$k]);
                    }
                }
            }
    ...
    public function actionGetStoreDetail(){
        ...
        if (isset($post["sNo"]) && !empty($post["sNo"])) {
            $query = $query->andWhere(["l.agent_id" => $post["sNo"]]);
        }
        if($type==1){
