...
<div>
    <form class="myForm" id="filterForm">
        <ul class="third_team_ul">
            <li>
                <label style="margin-left:15px;" for="">代理商信息:</label>
                <input type="text" id="agentName" class="form-control" placeholder="代理商编号、代理商名称" style="width:250px;display: inline-block;" >
            </li>
            <li>
                <label>统计时间:</label>
    ...
    <form class="myForm" id="filterForm1" style="display:none">
        <ul class="third_team_ul" >
            <li>
                <label style="margin-left:15px;" for="">代理商信息:</label>
                <input type="text" id="agentNames" class="form-control" placeholder="代理商编号、代理商名称" style="width:250px;display: inline-block;" >
            </li>
            <li>
                <label>统计时间:</label>
    ...
    <table class="table" id="pwTable">
        <thead>
            <tr>
                ...
                <th style="text-align: center;">代理商编号</th>
...
//    获取日统计数据
    function getReport() {
        ...
        if (timestamp1 < timestamp2) {
            ...
            $.ajax({
                url: "/report/agent-report/get-report",
                type: "POST",
                data: {start_date: star, end_date: end, agentName: agentName,type:type},
                async: false,
                dataType: "json",
                success: function (json) {
                    ...
                    $.each(json["result"], function (key, val) {
                        ...
                        html += "<td style='text-align: center'>" + val.agents_account + "</td>"
                        html += "<td style='text-align: center'>" + val.agents_name + "</td>"
                        ...
                        html += "<td style='text-align: center' ><a onclick='location.href = \"/report/agent-report/store-detail?type="+type+"&days=" + val.days + "&agent=" + val.agents_name +"&sNo=" + val.agents_id + "\"'>查看门店出票详情</a></td>"
                        html += "</tr>"
                    });
                    html += "<tr style='font-weight:bold;background-color:#E9ECF3'><td style='text-align: center;font-size:16px'>统计</td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'>" + counts + "</td><td style='text-align: center;font-size:16px'>" + ordernums + "</td><td style='text-align: center;font-size:16px'>" + noopen + "</td><td style='text-align: center;font-size:16px'>" + noaward + "</td><td style='text-align: center;font-size:16px'>" + salemoneys + "</td><td style='text-align: center;font-size:16px'>" + paymoneys + "</td><td style='text-align: center;font-size:16px'>" + winmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'>"+ awardmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'></td></tr>"
                    if (html == '') {
...
//    获取月统计数据
    function getMonthReport() {
        var agentName = $("#agentNames").val();
        var years = $("#years").val();
        var months = $("#months").val();
        var type = $("#type2").val();
        $.ajax({
            url: "/report/agent-report/get-month-report",
            type: "POST",
            data: {years: years,months:months,agentName: agentName,type:type},
            async: false,
            dataType: "json",
            success: function (json) {
                if (json["result"] == "") {
                ...
                $.each(json["result"], function (key, val) {
                    ...
                    html += "<td style='text-align: center'>" + val.winmoney + "</td>"
                    html += "<td style='text-align: center'>" + (val.award_amount!=null?val.award_amount:'0.00') + "</td>"
                    html += "<td style='text-align: center' ><a onclick='location.href = \"/report/agent-report/store-detail?type="+type+"&months=" + val.months + "&agent=" + val.agents_name +"&sNo=" + val.agents_id + "\"'>查看门店出票详情</a></td>"
                    html += "</tr>"
                });
                html += "<tr style='font-weight:bold;background-color:#E9ECF3'><td style='text-align: center;font-size:16px'>统计</td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'>" + counts + "</td><td style='text-align: center;font-size:16px'>" + ordernums + "</td><td style='text-align: center;font-size:16px'>" + noopen + "</td><td style='text-align: center;font-size:16px'>" + noaward + "</td><td style='text-align: center;font-size:16px'>" + salemoneys + "</td><td style='text-align: center;font-size:16px'>" + paymoneys + "</td><td style='text-align: center;font-size:16px'>" + winmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'>"+ awardmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'></td></tr>"
                if (html == '') {
    ...
    function getLotteryReport() {
        ...
        var type = $("#type3").val();
        $.ajax({
            url: "/report/agent-report/get-lottery-report",
            type: "POST",
            ...
            success: function (json) {
                ...
                $.each(json["result"], function (key, v) {
                    $.each(v,function (k,val) {
                        ...
                        html += "<td style='text-align: center'>" + val.winmoney+ "</td>"
                        html += "<td style='text-align: center'>" + (val.award_amount!=null?val.award_amount:'0.00') + "</td>"
                        html += "<td style='text-align: center' ><a onclick='location.href = \"/report/agent-report/store-detail?type="+type+"&star=" + star +"&end=" + end +"&lottery=" + val.lottery_id +"&lottery_name=" + val.lottery_name+"&channel=" + val.agents_name+"&sNo=" + val.agents_id+"\"'>查看门店出票详情</a></td>"
                        html += "</tr>"
                    })
                    });
                html += "<tr style='font-weight:bold;background-color:#E9ECF3'><td style='text-align: center;font-size:16px'>统计</td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'></td><td style='text-align: center;font-size:16px'>" + counts + "</td><td style='text-align: center;font-size:16px'>" + ordernums + "</td><td style='text-align: center;font-size:16px'>" + noopen + "</td><td style='text-align: center;font-size:16px'>" + noaward + "</td><td style='text-align: center;font-size:16px'>" + salemoneys + "</td><td style='text-align: center;font-size:16px'>" + paymoneys + "</td><td style='text-align: center;font-size:16px'>" + winmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'>"+ awardmoneys.toFixed(2) + "</td><td style='text-align: center;font-size:16px'></td></tr>"
