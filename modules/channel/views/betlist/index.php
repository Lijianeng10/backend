...
echo '<ul class="nav nav-tabs" style="margin-bottom:10px;">
  <li role="presentation"' . ((!isset($_GET['lottery_category_id']) || $_GET['lottery_category_id'] == '4,5,6') ? ' class="active"' : '') . '><a href="/channel/betlist?lottery_category_id=4,5,6">数字彩</a></li>
  <li role="presentation"' . ((isset($_GET['lottery_category_id']) && $_GET['lottery_category_id'] == '11') ? ' class="active"' : '') . '><a href="/channel/betlist?lottery_category_id=11">足球竞彩</a></li>
  <li role="presentation"' . ((isset($_GET['lottery_category_id']) && $_GET['lottery_category_id'] == '12') ? ' class="active"' : '') . '><a href="/channel/betlist?lottery_category_id=12">篮球竞彩</a></li>
  <li role="presentation"' . ((isset($_GET['lottery_category_id']) && $_GET['lottery_category_id'] == '13') ? ' class="active"' : '') . '><a href="/channel/betlist?lottery_category_id=13">北京单场</a></li>
</ul>';
?>
<div style="font-size:14px;">
    ...
    echo Html::input("input", "third_order_code", isset($get["third_order_code"]) ? $get["third_order_code"] : "", ["id" => "third_order_code", "class" => "form-control", "placeholder" => "商户订单号", "style" => "width:200px;display:inline;margin-left:5px;"]);
    echo '</li>';
    echo '<li>';
    ...
    echo '</li>';
    echo '<li>';
    echo Html::label("渠道会员", "user_info", ["style" => "margin-left:15px;"]);
    echo Html::input("input", "user_info", isset($get["user_info"]) ? $get["user_info"] : "", ["id" => "user_info", "class" => "form-control", "placeholder" => "会员编号、会员手机号、会员名称", "style" => "width:220px;display:inline;margin-left:5px;"]);
    echo '</li>';
    echo '<li>';
    echo Html::label("门店信息", "store_info", ["style" => "margin-left:15px;"]);
    echo Html::input("input", "store_info", isset($get["store_info"]) ? $get["store_info"] : "", ["id" => "store_info", "class" => "form-control", "placeholder" => "门店编号、门店注册手机号、门店名称", "style" => "width:230px;display:inline;margin-left:5px;"]);
    echo '</li>';
    echo '<li>';
    ...
    echo Html::dropDownList("out_status", isset($get["out_status"]) ? $get["out_status"] : "", $orderStatus, ["id" => "out_status", "class" => "form-control", "style" => "width:100px;display:inline;margin-left:5px;"]);
    echo '</li>';
    echo '<li>';
            ...
            [
                ...
                'value' => function($model) {
                    return '<div class="am-btn-group am-btn-group-xs">
                            <span class="handle pointer" onclick="readDeatail(\'/channel/betlist/readdetail12?lottery_order_id=' . $model['lottery_order_id'] . '\')">查看</span>' . 
                            (($model['status'] == 2 && $_SESSION['type'] == 0) ?  ('<span class="handle pointer" onclick="cancelOrder('.$model['lottery_order_id'].');">| 撤单</span>')  : "" ) .
                           '</div>';
                }
            ]
        ]
    ...
    echo GridView::widget([
        ...
        "columns" => [
            ...
            [
                ...
                'value' => function($model) {
                    return '<div class="am-btn-group am-btn-group-xs">
                            <span class="handle pointer" onclick="readDeatail(\'/channel/betlist/readdetail3?lottery_order_id=' . $model['lottery_order_id'] . '\')">查看 </span>' . 
                            (($model['status'] == 2 && $_SESSION['type'] == 0) ?  ('<span class="handle pointer" onclick="cancelOrder('.$model['lottery_order_id'].');">| 撤单</span>')  : "" ) .
                           '</div>';
        }
            ]
            ...
            [
                ...
                'value' => function($model) {
                    return '<div class="am-btn-group am-btn-group-xs bigRead">
                                    <span class="handle pointer readLan" onclick="readDeatail(\'/channel/betlist/readdetail4?lotteryOrderCode=' . $model['lottery_order_code'] . '\')">查看</span>' . 
                            (($model['status'] == 2 && $_SESSION['type'] == 0) ?  ('<span class="handle pointer" onclick="cancelOrder('.$model['lottery_order_id'].');">| 撤单</span>')  : "" ) .
                           '</div>';
                }
            ]
...
//                 ' . ($model['status'] == 3 ?  ('<span class="handle pointer" onclick="awardSubmit('.$model['lottery_order_id'].');">开奖</span>')  : "" ) . '
            return '<div class="am-btn-group am-btn-group-xs bigRead">
                           <span class="handle pointer readLan" onclick="readDeatail(\'/channel/betlist/read-bd-detail?lotteryOrderCode=' . $model['lottery_order_code'] . '\')">查看 </span>' . 
                            (($model['status'] == 2 && $_SESSION['type'] == 0) ?  ('<span class="handle pointer" onclick="cancelOrder('.$model['lottery_order_id'].');">| 撤单</span>')  : "" ) .
                           '</div>';
        }
            ]
...
<script type="text/javascript">
    ...
    function goReset() {
        location.href = '/channel/betlist/index<?php echo isset($_GET['lottery_category_id']) ? ("?lottery_category_id=" . $_GET['lottery_category_id']) : "" ?>';
    }
    function search() {
        ...
        location.href = '/channel/betlist/index' + param;
    }
...
//                    console.log(data);
    ...
    function cancelOrder(orderId) {
        msgConfirm ('提醒','确定要撤销此单？',function() {
            $.ajax({
                url:"/channel/betlist/channel-cancel",
                async: false,
                type: 'POST',
