...
//                $val["content"][$key]["rf_nums"] = $schedules[$mid]["sfgg_rq_nums"];
     ...
     * @return type
     */
    public function actionChannelCancel() {
        $request = \Yii::$app->request;
        $orderId = $request->post('orderId', '');
        if(empty($orderId)) {
            return $this->jsonError(100, '参数缺失');
        }
        $orderData = LotteryOrder::findOne(['lottery_order_id' => $orderId, 'status' => 2]);
        if(empty($orderData)) {
            return $this->jsonError(109, '该订单不符合撤单要求');
        }
        $data = ['orderCode' => $orderData->lottery_order_code];
        $url = \Yii::$app->params['userDomain'] . '/api/orders/order/cancel-order';
        $ret = \Yii::sendCurlPost($url, $data);
        if($ret['code'] != 600) {
            return $this->jsonError(109, $ret['msg']);
        }
        $orderData->opt_id = \Yii::$app->session['admin_id'];
