 ...
 */
class PcBananerController extends Controller {
    ...
    public function actionIndex() {
        ...
        $bananer=  Bananer::find()->where(["type"=>4]);
        if (!empty($picName)) {
...
//           $bananer->status= $status;
           ...
           if ($bananer->validate()) {
                $result = $bananer->save();
                if ($result == false) {
                    return $this->jsonResult(109, 'PC首页广告新增失败');
                }
                return $this->jsonResult(600, 'PC首页广告新增成功');
            } else {
    ...
    public function actionEditStatus() {
        if (!Yii::$app->request->isAjax) {
            return $this->redirect('/website/pc-bananer/index');
        }
        $request = Yii::$app->request;
...
//           $bananer->status= $status;
           ...
           if ($bananer->validate()) {
                $result = $bananer->save();
                if ($result == false) {
                    return $this->jsonResult(109, 'PC首页广告编辑失败');
                }
                return $this->jsonResult(600, 'PC首页广告编辑成功');
            } else {
                return $this->jsonResult(109, 'Bananer表单验证失败', $bananer->errors);
            }
        }
