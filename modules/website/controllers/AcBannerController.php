 ...
 */
class AcBannerController extends Controller {
    ...
    public function actionIndex() {
        ...
        $bananer=  Bananer::find()->where(["type"=>6]);
        if (!empty($picName)) {
    ...
    public function actionAddBananer() {
        if (Yii::$app->request->isGet) {
            ...
            return $this->render("add-banner", ["picType" => $picType, "picStatus" => $picStatus,"picUse"=>$picUse,"picArea"=>$picArea,"picJump"=>$picJump]);
...
//            $bananer->area= $area;
    ...
    public function actionEditStatus() {
        if (!Yii::$app->request->isAjax) {
            return $this->redirect('/website/ac-banner/index');
        }
        $request = Yii::$app->request;
...
//        $this->layout = false;
        if (Yii::$app->request->isGet) {
            ...
            return $this->render('edit-banner',["data"=>$bananer,"picType" => $picType, "picStatus" => $picStatus,"picUse"=>$picUse,"picArea"=>$picArea,"picJump"=>$picJump]);
        }elseif(Yii::$app->request->isAjax){
            $post = Yii::$app->request->post();
            $session = Yii::$app->session;
