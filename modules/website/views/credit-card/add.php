    ...
    <form class="add_card">
        <div>
            <div>
                ...
                <label>
                    <input name="cardName" id="cardName" type="text" class="form-control need" placeholder="信用卡名称" style="width: 200px;">
                </label>
            </div>
            <div>
                <span class="infoSpan" style="display: inline-block;padding-left: 10px;">取现额度</span>
                <label>
                    <input name="cashQuota" id="cashQuota" type="text" class="form-control" placeholder="取现额度 类：30%" style="width: 200px;">
                </label>
            </div>
            <div>
                <span class="infoSpan" style="display: inline-block;padding-left: 10px;">免息期<span style="color: red">*</span></span>
                <label>
                    <input name="freePeriods" id="freePeriods" type="text" class="form-control need" placeholder="免息期 类：30天" style="width: 200px;">
                </label>
            </div>
            ...
            <div>
                ...
                <label>
                    <input name="sort" id="sort" type="number" class="form-control minZero" placeholder="显示优先级 数值越小 显示越靠前" style="width: 200px;" min="0" max="100" value="100">
                </label>
            </div>
            ...
            <div style="margin-top:5px;">
                ...
                <label>
                    <textarea name="cardActivity" id="cardActivity" style="width:300px;height: 100px;solid:1px,#c2cad8; border-radius:4px;" placeholder="如果有多个活动请以’;‘ 隔开"></textarea>
                </label>
            </div>
...
<script>
    ...
    $("#addActivity").click(function() {
        var tr = '';
        tr = '<tr><td><input type="text"></td><tr>';
        $("#addConspan").before(tr);
    })
    ...
    $('#addSubmit').click(function () {
        ...
        var data = $(".add_card").serializeArray();
        formData.append("upfile", $("#uploadPic").get(0).files[0]);
        $.each(data, function (i, field) {
            formData.append(field.name, field.value);
        });
        $.ajax({
            url: '/website/credit-card/add',
            async: false,
            type: 'POST',
            processData: false,
            contentType: false,
