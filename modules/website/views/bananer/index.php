...
<div style="font-size:14px;">
    <form action="/website/bananer/index">
        <ul class="third_team_ul">
            ...
            echo '</li>';
            echo '<li>';
            echo Html::label("资讯类型  ", "", ["style" => "margin-left:15px;"]);
            echo Html::dropDownList("infoType", isset($get["infoType"]) ? $get["infoType"] : "", $infoType, ["class" => "form-control", "style" => "width:150px;display:inline;margin-left:5px;"]);
            echo '</li>';
            echo '<li>';
            echo Html::label("所属联赛  ", "", ["style" => "margin-left:15px;"]);
            echo Html::dropDownList("leagueCode", isset($get["leagueCode"]) ? $get["leagueCode"] : "", $leagueCode, ["class" => "form-control", "style" => "width:150px;display:inline;margin-left:5px;"]);
            echo '</li>';
            echo '<li>';
            ...
            'value' => function($model) {
                if (!empty($model['pic_url'])) {
                    ...
                    'value' => function($model) {
                        ...
                        return $infoType[$model["info_type"]];
                    }
                ],[
                    'label' => '所属类型',
                    'value' => 'league_code'
...
<script>
    ...
    //新增
    function addBananer() {
        location.href ='/website/bananer/add-bananer'
//        modDisplay({title: '新增广告', url: '/website/bananer/add-bananer', height: 780, width: 800});
    ...
    function delBananer(id) {
        msgConfirm('提醒',"您确定要删除该广告吗", function () {
            $.ajax({
                url: "/website/bananer/del-bananer",
                type: "POST",
                async: false,
                data: {bananer_id: id},
                dataType: "json",
    ...
    function editBananer(id){
        location.href ='/website/bananer/edit-bananer?bananer_id='+id
