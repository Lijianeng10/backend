    ...
    $(function () {
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                var taking = json["result"]["takingRecord"];
                ...
                t1 += "<tr><th>出票手续费</th><td>" + (data.pay_pre_money?data.pay_pre_money:"") + "</td></tr>";
                t1 += "</table>";
                ...
                t2 += "<tr><th>退款号</th><td>" + (payInfo.refund_no?payInfo.refund_no:"") + "</td></tr>";
                    ...
                    $.each(data["contents"], function (key, val) {
                        ...
                        if (val.result_status == 3) {
                            ...
                            $.each(playAry, function (k, v) {
                                if (val.hasOwnProperty("schedule_result_5001") && val.schedule_result_5001 != ""&& v==5001) {
                                    playHtml += "<span style='text-align: center;display:block;'>"+Array5001[val.schedule_result_5001]+"</span>"
                                }
                                if (val.hasOwnProperty("schedule_result_5002") && val.schedule_result_5002 != ""&& v==5002) {
                                    playHtml += "<span style='text-align: center;display:block;'>"+Array5002[val.schedule_result_5002]+"</span>"
                                }
                                if (val.hasOwnProperty("schedule_result_5003")&& val.schedule_result_5003 != ""&& v==5003) {
                                    playHtml += "<span style='text-align: center;display:block;'>"+Array5003[val.schedule_result_5003]+"</span>";
                                }
                                if (val.hasOwnProperty("schedule_result_5004") && val.schedule_result_5004 != ""&& v==5004) {
                                    playHtml += "<span style='text-align: center;display:block;'>"+Array5004[val.schedule_result_5004]+"</span>";
                                }
                                if (val.hasOwnProperty("schedule_result_5005")&& val.schedule_result_5005 != ""&& v==5005) {
...
//                            competHtml += "<img class='orderImg' src='" + picAry[i] + "'>";
                    ...
                    competHtml += '<tr><td style="vertical-align: middle;">拒绝理由</td><td>' + (data.refuse_reason? data.refuse_reason : "") + '</td></tr>';
                    var conHtml = "";
                    conHtml += competHtml;
//                    
                 }
                ...
                //接单记录
                var t5 = "<table class='table table-striped table-bordered 'style='width:92%;font-size: 12px;'>";
                t5 += "<tr><th  style='text-align: center;'>#</th><th  style='text-align: center;'>门店名称</th><th style='text-align: center;'>门店编号</th><th  style='text-align: center;'>接单状态</th><th  style='text-align: center;'>创建时间</th><th  style='text-align: center;'>更新时间</th></tr>";
                $.each(taking, function (key, val) {
                    t5 += "<tr style='text-align: center;'><td>" + eval(key + 1) + "</td>";
                    t5 += "<td style='width:140px;'>"+val.store_name+"</td>";
                    t5 += "<td>" + val.store_code + "</td>";
                    if (val.status==0) {
                        t5 += "<td>暂未接单</td>";
                    ...
                    t5 += "<td>" + val.create_time + "</td>";
                    t5 += "<td>" + val.modify_time + "</td>";
                    t5 += "</tr>";
                })
                t5 += "</table>";

                var t4 = "<table class='table table-striped table-bordered modalTable'style='width:92%;margin-top: 10px;' id='detailList'>";
        ...
        $("#content").on("click", "#getDetailMore", function () {
            ...
            $.ajax({
                url: "/lottery/betting/get-bd-more-detail",
                async: false,
                type: 'POST',
                data: data,
                dataType: "json",
