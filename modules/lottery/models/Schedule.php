...
class Schedule extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasOne(Odds3010::className(), ['schedule_mid' => 'schedule_mid'])
            ->orderBy('odds_outcome_id desc');
    }
    public function getOdds3006()
    {
        return $this->hasOne(Odds3006::className(), ['schedule_mid' => 'schedule_mid'])
//         ->select('let_ball_nums', 'let_wins', 'let_level', 'let_negative')
        ->orderBy('odds_let_id desc');
    }
    public function getOdds3007()
    {
        return $this->hasOne(Odds3007::className(), ['schedule_mid' => 'schedule_mid'])
        ->orderBy('odds_score_id desc');
    }
    public function getOdds3008()
    {
        return $this->hasOne(Odds3008::className(), ['schedule_mid' => 'schedule_mid'])
        ->orderBy('odds_3008_id desc');
    }
    public function getOdds3009()
    {
        return $this->hasOne(Odds3009::className(), ['schedule_mid' => 'schedule_mid'])
