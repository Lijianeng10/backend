...
class TogetherService {
    ...
    public static function getAllProgramme($pn, $size, $by, $code) {
        ...
        foreach ($data as &$val) {
            ...
            $perOrderSpeed = floor(($val['minimum_guarantee'] / $val['bet_money'] ) * 100) . '%';
     ...
     * @return type
     */
    public static function getSubscribeDetail($pId = '', $storeId, $isWith = 0, $programmeCode = '') {
        $query = new Query;
        $date = date('Y-m-d H:i:s');
        $status = Constants::PROGRAMME_STATUS;
        ...
        $field = ['p.programme_id', 'o.lottery_order_id', 'p.programme_code', 'p.expert_no', 'p.cust_type', 'p.store_id', 'p.lottery_name', 'p.lottery_code', 'p.periods', 'p.bet_val', 'p.play_code', 'p.play_name', 'p.bet_double', 'p.is_bet_add',
            ...
            'el.level', 'p.programme_last_number', 'p.programme_univalent', 'p.made_nums'];
        if ($pId != '') {
        ...
        $data = $query->select($field)
                ->from('programme as p')
                ->leftJoin('lottery_order as o', 'o.programme_code = p.programme_code')
        ...
        if (in_array($data['lottery_code'], $football) && !empty($data['count'])) {
            if ($data['security'] == 1 || ($data['store_id'] == $storeId) || in_array($data['status'], [4, 5, 6]) || ($data['security'] == 2 && $isWith == 1) || ($data['security'] == 3 && (strtotime($date) > strtotime($data['programme_end_time'])))) {
                $odds = [];
                if (in_array($data['status'], [3, 4, 5, 6])) {
                    $oddsArr = (new Query)->select(['odds'])->from('lottery_order')->where(['programme_code' => $data['programme_code']])->one();
                    if (!empty($oddsArr)) {
        ...
        $perOrderSpeed = floor(($data['minimum_guarantee'] / $data['bet_money'] ) * 100) . '%';
        $data['per_order_speed'] = $perOrderSpeed;
        $data['status_name'] = $status[$data['status']];

        $prize['win_amount'] = $data['win_amount'];
        $prize['commission'] = $data['win_amount'] * ($data['royalty_ratio'] / 100);
     ...
     * @return type
     */
    public static function getListDetail($programmeCode) {
        $query = new Query;
        $date = date('Y-m-d H:i:s');
        $status = Constants::PROGRAMME_STATUS;
        $securityArr = Constants::PROGRAMME_SECURITY;
        ...
        $field = ['p.programme_id', 'p.programme_code', 'p.expert_no', 'p.cust_type', 'p.store_id', 'p.lottery_name', 'p.lottery_code', 'p.periods', 'p.bet_val', 'p.play_code', 'p.play_name', 'p.bet_double', 'p.is_bet_add',
            ...
            'el.made_nums', 'el.win_nums', 'el.win_amount as all_win_amount', 'el.level', 'p.programme_last_number', 'p.programme_univalent', 'p.made_nums'];
        if ($programmeCode != '') {
            $where['p.programme_code'] = $programmeCode;
        }
        $data = $query->select($field)
        ...
        $data['prize'] = null;
        //获取更单人员信息
        $withPeople=(new Query())->select("cust_no,user_name,bet_money,buy_number,win_amount,create_time,programme_user_code")
                ->from("programme_user")
                ->where(["programme_code"=>$programmeCode])
                ->andWhere("status!=1")
                ->all();
        $data["withPeople"]=$withPeople;
        return ['code' => 600, 'msg' => '方案详情', 'data' => $data];
    }
            ...
            if ($code != '3011' && $code != '3005') {
                ...
                foreach ($arr as $v) {
                    ...
                    if (in_array($code, $football)) {
                        ...
                        $bets[$key]["lottery"][$n]["odds"] = isset($odds[$code][$result[1]][$v]) ? $odds[$code][$result[1]][$v] : $basketService->getOdds($code, $result[1], [$v]); //赔率
                    }
                    $n++;
                }
            } else {
                ...
                foreach ($strs as $str) {
                    ...
                    foreach ($arr as $v) {
                        ...
                        if (in_array($code, $football)) {
                            ...
                            $basketService = new BasketService();
                            $bets[$key]["lottery"][$n]["odds"] = isset($odds[$r[1]][$result[1]][$v]) ? $odds[$r[1]][$result[1]][$v] : $basketService->getOdds($r[1], $result[1], [$v]); //赔率
                        }
                        $n++;
                    }
                }
...
//            $val["visit_team_name"] = $schedule["visit_team_name"];
    ...
    public static function getWithPeople($pn, $size, $programmeId = '', $programmeCode = '') {
        ...
        $offset = ($pn - 1) * $size;
        $status = Constants::PROGRAMME_STATUS;
        $withPeople = ProgrammeUser::find()->select(['user_name', 'bet_money', 'buy_number', 'win_amount', 'create_time', 'status'])->where($where)->andWhere($sta)->limit($size)->offset($offset)->orderBy('buy_number desc')->asArray()->all();
